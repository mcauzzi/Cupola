...
int main()
{
	...
	// Search for a Module-file like "Type0014.md3".
#if defined( _WIN32 )
...
#endif
	if ( bRet == false ) {
		puts( "\"Type0014 Module\" is not found.\n" );
		return -1;
	}
...
#endif
	if ( bRet == false ) {
		puts( "Failed in loading \"Type0014 Module\".\n" );
		return -1;
	}
...
BOOL SourceCommandLoop( LPRefObj pRefMod, ULONG ulSrcID )
{
	...
	do {
		...
		printf( " 4. Shooting Menu            5. Live View                6. Custom Menu\n" );
		printf( " 7. Async                    8. Autofocus                9. Capture\n" );
		printf( "10. TerminateCapture        11. PreCapture\n" );
		printf( " 0. Exit\n>" );
		scanf( "%s", buf );
		wSel = atoi( buf );
...
BOOL SetUpCamera1( LPRefObj pRefSrc ) 
{
	...
	do {
		...
		printf( "19. FocalLength             20. ClockDateTime            21. SaveMedia\n" );
		printf( " 0. Exit\n>" );
		scanf( "%s", buf );
		wSel = atoi( buf );
...
BOOL SetUpCamera2( LPRefObj pRefSrc ) 
{
	...
	// Connect D810 camera
	if ( g_ulCameraType == kNkMAIDCameraType_D810)
	{
		do {
			// Wait for selection by user
			printf( "\nSelect the item you want to set up\n" );
	...
	// Connect D810A camera
	else if ( g_ulCameraType == kNkMAIDCameraType_D810A)
	{
		do {
			// Wait for selection by user
			printf( "\nSelect the item you want to set up\n" );
			printf( " 1. LockCamera             2. LensInfo                  3. UserComment\n" );
			printf( " 4. EnableComment          5. IsoControl                6. NoiseReduction\n" );
			printf( " 7. MirrorUpCancel         8. MirrorUpStatus            9. MirrorUpReleaseShootingCount\n" );
			printf( " 0. Exit\n>" );
			scanf( "%s", buf );
			wSel = atoi( buf );
			...
			switch( wSel )
			{
				...
				case 6:// NoiseReduction 
					bRet = SetBoolCapability( pRefSrc, kNkMAIDCapability_NoiseReduction );
					break;
				case 7:// MirrorUpCancel 
					bRet = IssueProcess( pRefSrc, kNkMAIDCapability_MirrorUpCancel);
					break;
				case 8:// MirrorUpStatus 
					bRet = SetUnsignedCapability( pRefSrc, kNkMAIDCapability_MirrorUpStatus );
					break;
				case 9:// MirrorUpReleaseShootingCount 
					bRet = SetUnsignedCapability( pRefSrc, kNkMAIDCapability_MirrorUpReleaseShootingCount );
					break;
				default:
					wSel = 0;
					break;
	...
	{
		bRet = false;
		printf( "Unknown camera. This camera is not supported.\n>" );
	}
...
BOOL SetShootingMenu( LPRefObj pRefSrc ) 
{
	...
	do {
		...
		printf( " 7. WBAutoType             8. WBTuneIncandescent      9. WBFluorescentType\n" );
		printf( "10. WBTuneFluorescent     11. WBTuneSunny            12. WBTuneFlash \n" );
		printf( "13. WBTuneShade           14. WBTuneCloudy           15. WBPresetData\n" );
		printf( "16. PictureControl        17. PictureControlData     18. GetPicCtrlInfo\n" );
		printf( "19. DeleteCustomPicCtrl   20. PictureControlDataEx   21. MovieMeteringMode\n");
		printf( " 0. Exit\n>" );
		scanf( "%s", buf );
		wSel = atoi( buf );
		...
		switch( wSel )
		{
			...
			case 13:// WBTuneShade
				...
				break;
			case 14:// WBTuneCloudy
				bRet = SetRangeCapability( pRefSrc, kNkMAIDCapability_WBTuneCloudy );
				break;
			case 15:// WBPresetData
				bRet = SetWBPresetDataCapability( pRefSrc );
				break;
			case 16:// PictureControl
				bRet = SetEnumCapability( pRefSrc, kNkMAIDCapability_PictureControl );
				break;
			case 17:// PictureControlData
				bRet = PictureControlDataCapability( pRefSrc, kNkMAIDCapability_PictureControlData);
				break;
			case 18:// GetPicCtrlInfo 
				bRet = GetPictureControlInfoCapability( pRefSrc );
				break;
			case 19:// DeleteCustomPictureControl
				bRet = DeleteCustomPictureControlCapability( pRefSrc );
				break;
			case 20:// PictureControlDataEx
				bRet = PictureControlDataCapability( pRefSrc, kNkMAIDCapability_PictureControlDataEx);
				break;
			case 21:// MovieMeteringMode
...
BOOL SetLiveView( LPRefObj pRefSrc ) 
{
	...
	do {
		// Wait for selection by user
		printf( "\nSelect the item you want to set up\n" );
		printf( " 1. LiveViewProhibit       2. LiveViewStatus          3. LiveViewImageZoomRate\n" );
		printf( " 4. LiveViewImageSize      5. GetLiveViewImage        6. MovRecInCardProhibit\n");
		printf( " 7. MovRecInCardStatus     8. SpotWBMode              9. SpotWBMeasure\n");
		printf( "10. SpotWBChangeArea      11. SpotWBResultDispEnd\n" );
		printf( " 0. Exit\n>" );
		scanf( "%s", buf );
		wSel = atoi( buf );
...
BOOL SetCustomSettings( LPRefObj pRefSrc ) 
{
	...
	do {
		...
		printf( " 1. EVInterval              2. BracketingVary         3. ExpBaseHighlight\n" );
		printf( " 4. ElectronicFrontCurtainShutter                     0. Exit\n>" );
		scanf( "%s", buf );
		wSel = atoi( buf );
		...
		switch( wSel )
		{
			...
			case 2:// BracketingVary
				...
				break;
			case 3:// ExpBaseHighlight
				bRet = SetRangeCapability( pRefSrc, kNkMAIDCapability_ExpBaseHighlight );
				break;
			case 4:// ElectronicFrontCurtainShutter
				bRet = SetEnumCapability( pRefSrc, kNkMAIDCapability_ElectronicFrontCurtainShutter );
				break;
			default:
				wSel = 0;
				break;
...
BOOL ItemCommandLoop( LPRefObj pRefSrc, ULONG ulItmID )
{
	...
	if ( pRefItm == NULL ) {
		...
		if ( AddChild( pRefSrc, ulItmID ) != false ) {
...
BOOL ImageCommandLoop( LPRefObj pRefItm, ULONG ulDatID )
{
	...
	if ( pRefDat == NULL ) {
		...
		if ( AddChild( pRefItm, ulDatID ) != false ) {
