...
BOOL Search_Module( void* Path )
{
	...
	CFStringRef		fileName = CFSTR("Type0014 Module.bundle");
...
{
	BOOL bRet = true;
	while( *pulCount < ulEndCount && bRet != false ) {
...
BOOL Command_CapStart(LPNkMAIDObject pobject, ULONG ulParam, LPNKFUNC pfnComplete, NKREF refComplete, SLONG* pnResult)
{
	...
	return ( nResult == kNkMAIDResult_NoError ||
			 nResult == kNkMAIDResult_Pending ||
			 nResult == kNkMAIDResult_BulbReleaseBusy || 
		     nResult == kNkMAIDResult_SilentReleaseBusy ||
			 nResult == kNkMAIDResult_MovieFrameReleaseBusy ||
			 nResult == kNkMAIDResult_Waiting_2ndRelease ||
			 nResult == kNkMAIDResult_MirrorUpCapture_Already_Start );
}
//------------------------------------------------------------------------------------------------------------------------------------
//
...
char* GetEnumString( ULONG ulCapID, ULONG ulValue, char *psString )
{
	switch ( ulCapID ) {
		...
		case kNkMAIDCapability_MeteringMode:
			switch( ulValue ){
				...
				case kNkMAIDMeteringMode_Spot:
					...
					break;
				case kNkMAIDMeteringMode_HighLight:
					strcpy( psString, "HighLight" );
					break;
				default:
		...
		case kNkMAIDCapability_SaveMedia:
			switch( ulValue ){
				case kNkMAIDSaveMedia_Card:
					strcpy( psString, "Card" );
					break;
				case kNkMAIDSaveMedia_SDRAM:
					strcpy( psString, "SDRAM" );
					break;
				case kNkMAIDSaveMedia_Card_SDRAM:
					strcpy( psString, "Card + SDRAM" );
					break;
				default:
					sprintf( psString, "SaveMedia %u", ulValue );
			}
			break;
		default:
...
BOOL GetLiveViewImageCapability( LPRefObj pRefSrc )
{
	...
	if ( (g_ulCameraType == kNkMAIDCameraType_D810) || (g_ulCameraType == kNkMAIDCameraType_D810A) )
	{
		ulHeaderSize = 384;
	}
	...
	do {
		...
		switch( ulSel )
		{
			case 1://Set Picture Control data 
			{
				...
				printf( " 7. Flat\n");
				printf( " 8. Custom Picture Contol 1     9. Custom Picture Contol 2 \n");
				printf( " 10. Custom Picture Contol 3    11. Custom Picture Contol 4 \n");
				printf( " 12. Custom Picture Contol 5    13. Custom Picture Contol 6 \n");
				printf( " 14. Custom Picture Contol 7    15. Custom Picture Contol 8 \n");
				printf( " 16. Custom Picture Contol 9\n");
				printf( " 0. Exit\n>" );
				scanf( "%s", buf );
				ulSubSel = atoi( buf );
				if ( ulSubSel == 0 ) break; //Exit
				if ( ulSubSel < 1 || ulSubSel > 16 ) 
				{
				...
				if ( ulSubSel >= 8 )
				{
					ulSubSel += 193; // Custom 201 - 209
				}
				// set target Picture Control
				stPicCtrlData.ulPicCtrlItem = ulSubSel;

				// initial registration is not supported about 1-7
				if ( (stPicCtrlData.ulPicCtrlItem >= 1 && stPicCtrlData.ulPicCtrlItem <= 7) )
				{
...
BOOL GetVideoImageCapability( LPRefObj pRefDat, ULONG ulCapID )
{
	...
	// write file
	while ( ( stVideoImage.ulOffset < ulTotalSize ) && ( bRet != false )  )
	{
		if( false != g_bCancel )
		{
			stVideoImage.ulDataSize = 0;
	...
	if( (stVideoImage.ulOffset < ulTotalSize) && ( false != g_bCancel) )
	{
